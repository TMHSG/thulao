  ...
  <v-row>
    ...
    <v-col cols="12">
      ...
      >
        ...
        <template v-slot:item.NgayKham="{ item }">
          {{
        ...
        <template v-slot:item.NgayKetHoSo="{ item }">
          {{
            new Date(item.NgayKetHoSo).toLocaleDateString('vi-VN', {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit',
            })
        ...
        <!-- <template v-slot:item.DonGiaThanhToan="{ item }">
          {{
            new Number(item.DonGiaThanhToan)
              .toString()
              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
          }}
        </template> -->
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Mã y tế', value: 'MaYTe' },
      { text: 'Tên bệnh nhân', value: 'TenBenhNhan' },
      { text: 'Ngày sinh', value: 'NgaySinh' },
      { text: 'Ngày khám', value: 'NgayKham' },
      { text: 'Ngày kết HS', value: 'NgayKetHoSo' },

      { text: 'Công ty', value: 'CongTy' },
      { text: 'Phạm vi', value: 'PhamVi' },
      { text: 'Ngoại viện', value: 'NgoaiVien' },
      { text: 'Bác sĩ', value: 'BacSi' },
      { text: 'Thù lao', value: 'ThuLao' },
      { text: 'Loại HS', value: 'LoaiHoSo' },
    ],
  }),
  computed: {
    formatTuNgay() {
  ...
  methods: {
    ...
    async getData() {
      ...
      if (
        ...
        this.congkham = await this.$axios
          .get(url)
    ...
    async exportToExcel() {
      const fileName = `915_TongKetHS_${this.formatDate(
        this.TuNgay
