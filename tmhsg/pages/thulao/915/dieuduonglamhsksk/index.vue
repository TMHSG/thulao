<template>
  <v-row>
    ...
    <v-col cols="12" sm="8" md="4" style="text-align: center; color: black"
      ><h2>ĐIỀU DƯỠNG LÀM HỒ SƠ KSK</h2></v-col
    >
    <v-col cols="12" sm="8" md="4"></v-col>
    <v-spacer></v-spacer>
    ...
    <v-col cols="12">
      ...
      >
        ...
        <!-- <template v-slot:item.GiaTriHoaDon="{ item }">
          {{
            new Number(item.GiaTriHoaDon)
              .toString()
              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
          }}
        </template> -->
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Ngày kết', value: 'NgayKetHoSo' },
      ...
      { text: 'Nhân viên', value: 'TenNhanVien' },
  ...
  methods: {
    ...
    async exportToExcel() {
      const fileName = `915_DieuDuongHSKSK_${this.formatDate(
        this.TuNgay
