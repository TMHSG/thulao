  ...
  <v-row>
    ...
    <v-col cols="12">
      <v-data-table
        ...
        class="elevation-1"
      >
        <template v-slot:item.NgayThucHien="{ item }">
          {{
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Tên bác sĩ', value: 'TenNhanVien' },
      { text: 'Tên dịch vụ', value: 'TenDichVu' },
      { text: 'Mức giảm', value: 'HopDongMucGiam' },
      { text: 'Id mức giảm', value: 'HopDongMucGiam_id' },
      { text: 'Tên mức giảm', value: 'TenMucGiam' },
      { text: 'Ngày', value: 'NgayThucHien' },
      { text: 'Công ty', value: 'TenDonViCongTac' },
  ...
  methods: {
    ...
    async exportToExcel() {
      const fileName = `915_CLSKSKDK_${this.formatDate(
        this.TuNgay
      ...
      this.congkham.forEach((item, index) => {
        ...
        const row = worksheet.addRow([
          ...
          item.TenDichVu,
          item.HopDongMucGiam,
          item.HopDongMucGiam_id,
          item.TenMucGiam,
          ngayKhamFormatted,
          item.TenDonViCongTac,
          item.Thulao,
        ])
        row.font = font
      })
