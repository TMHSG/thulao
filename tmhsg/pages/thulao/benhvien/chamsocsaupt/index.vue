<template>
  <v-row>
    ...
    <v-col cols="12" sm="8" md="4" style="text-align: center; color: black"
      ><h2>CHĂM SÓC SAU PHẪU THUẬT</h2></v-col
    >
    <v-col cols="12" sm="8" md="4"></v-col>
    <v-spacer></v-spacer>
    ...
    <v-col cols="12">
      <v-data-table
        ...
        class="elevation-1"
      >
        <template v-slot:item.ThoiGianGhiNhan="{ item }">
          {{
            new Date(item.ThoiGianGhiNhan).toLocaleDateString('vi-VN', {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit',
            })
...
export default {
  ...
  methods: {
    ...
    async exportToExcel() {
      const fileName = `BV_chamsocsaupt_${this.formatDate(
        this.TuNgay
