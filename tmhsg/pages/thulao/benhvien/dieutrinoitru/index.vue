<template>
  <v-row>
    ...
    <v-col cols="12" sm="8" md="4" style="text-align: center; color:black"><h2>ĐIỀU TRỊ NỘI TRÚ</h2></v-col>
    <v-col cols="12" sm="8" md="4"></v-col>
    <v-spacer></v-spacer>
    ...
    <v-col cols="12">
      <v-data-table
        ...
        class="elevation-1"
      >
        <template v-slot:item.NgayRaVien="{ item }">
          {{
            new Date(item.NgayRaVien).toLocaleDateString("vi-VN", {
              year: "numeric",
              month: "2-digit",
              day: "2-digit",
            })
      ...
      <div v-if="isLoading" class="loading-message" style="color:black; text-align: center"><h2>Vui lòng chờ...</h2></div>
    </v-col>
  </v-row>
</template>
...
export default {
  ...
  data: () => ({
    ...
    DenNgayMenu: false,
    isLoading: false,
    selectedNhanVien: null,apiUrl : process.env.API_BASE_URL,
    NhanVien_Id: null,
    ListNhanVien: [],
  ...
  methods: {
    async getListNhanVien() {
      try {
        const response = await axios.get(`${this.apiUrl}/nhanvien`);
    ...
    async getData() {
      ...
      if (
        ...
        this.dieutrinoitru = await axios
          .get(url)
    ...
    async exportToExcel() {
      const fileName = `BV_dieutrinoitru_${this.formatDate(this.TuNgay)}_${this.formatDate(
        this.DenNgay
      )}.xlsx`;
