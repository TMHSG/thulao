<template>
  <v-container>
    ...
    >
      <template v-slot:item.Hang="{ item }">
        <v-chip :style="{ 'min-width': '30px' }" light>
          {{ item.Hang }}
        </v-chip>
      </template>
      ...
      <template v-slot:top>
        <v-toolbar flat >
          <v-toolbar-title>Hạng bác sĩ</v-toolbar-title>
    ...
    <v-dialog v-model="dialogAdd" max-width="1000px">
      <v-card>
        <v-card-title>
          <span class="text-h5">Thêm hạng bác sĩ</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row class="mb-4">
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Id', value: 'Hang_Id' },
      {text: 'Hạng', value: 'Hang'},
  ...
  try {
    ...
    async saveEdit() {
      try {
        const response = await axios.put(
          `${this.apiUrl}/updatehangbs/${this.editItems.Hang_Id}`,
          this.editItems,
          { validateStatus: false }
        )
    ...
    async deleteItem() {
      try {
        ...
        const deleteResponse = await axios.delete(
          `${this.apiUrl}/xoahangbs/${this.selectedNhanVienId}`
        )
    ...
    exportToExcel() {
      ...
      const columns = [
        ...
        { header: 'Id', key: 'Hang_Id', width: 10 },
        { header: 'Hạng', key: 'Hang'},
