<template>
  <v-row>
    ...
    <v-col cols="12" sm="8" md="4" style="text-align: center; color: black"
      ><h2>THỰC HIỆN CẬN LÂM SÀN BH</h2></v-col
    >
    <v-col cols="12" sm="8" md="4"></v-col>
    <v-spacer></v-spacer>
    ...
    <v-col cols="12">
      ...
      >
        ...
        <template v-slot:item.DonGiaHoTroChiTra="{ item }">
          {{
            new Number(item.DonGiaHoTroChiTra)
              .toString()
              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
          }}
        </template>
...
export default {
  ...
  methods: {
    ...
    async exportToExcel() {
      const fileName = `BV_thclsbh_${this.formatDate(
        this.TuNgay
