<template>
  <v-row>
    ...
    <v-col cols="12" sm="8" md="4" style="text-align: center; color: black"
      ><h2>TỔNG HỢP KSK BS KẾT LUẬN</h2></v-col
    >
    <v-col cols="12" sm="8" md="4"></v-col>
    <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    calculateTongThuLao() {
      ...
      this.totalThuLaoFormatted = this.totalThuLao
        .toFixed(2)
        .replace(/\B(?=(\d{3})+(?!\d))/g, ',') // Định dạng với dấu phẩy và hai chữ số thập phân
    },
    ...
    async exportToExcel() {
      const fileName = `Newhis_TongHopKSK_BSKETLUAN_${this.formatDate(
        this.TuNgay
